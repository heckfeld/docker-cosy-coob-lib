#
# $Header: /mnt/cc-x3/coob/lib/steerer/rcs/wedel.tcl,v 1.3 2012/01/26 10:33:28 tine Exp $
#

itcl_class wedel_class {
  ...
  constructor {k} {
    ...
    set maxlen 5
    set entry_time_list [list begin up1 top down ntop up2]
    set entry_list [list topstrom begin up1 top down ntop up2]
  ...
  method rechne {num args} {
    ...
    if {[llength $args] <= 1} {
      ...
      foreach line $elem_list {
        ...
        set ntopstrom($elem) [expr $otopstrom($elem) - [set \
                              ${initial}topstrom($num)] * [$elem get_$scale_proc \
                              $num]]
      }
      ...
      set topstrom($elem_list) [expr [set ${initial}topstrom($num)] * [$elem_list \
                                get_$scale_proc $num] + $otopstrom($elem_list)]
      set ntopstrom($elem_list) [expr $otopstrom($elem_list) - \
                                 [set ${initial}topstrom($num)] * [$elem_list \
                                  get_$scale_proc $num]]
      set begin($num) [lindex $liste 3]
      set up($num) [lindex $liste 4]
      set wbegin($num) [lindex $liste 5]
      set wup1($num) [lindex $liste 6]
      set wtop($num) [lindex $liste 7]
      set wdown($num) [lindex $liste 8]
      set wntop($num) [lindex $liste 9]
      set wup2($num) [lindex $liste 10]
      set wend($num) [lindex $liste 11]
      set down($num) [lindex $liste 12]
      set end($num) [lindex $liste 13]
    }
    ...
    foreach line $elem_list {
      ...
      set rc [catch {exec $script $dir/$elem.fgen$num \
             ...
             [expr int($wbegin($num)*1000)]:$otopstrom($elem) \
             [expr int($wup1($num)*1000)]:$topstrom($elem) \
             [expr int($wtop($num)*1000)]:$topstrom($elem) \
             [expr int($wdown($num)*1000)]:$ntopstrom($elem) \
             [expr int($wntop($num)*1000)]:$ntopstrom($elem) \
             [expr int($wup2($num)*1000)]:$otopstrom($elem) \
             [expr int($wend($num)*1000)]:$otopstrom($elem) \
  ...
  method eingabe_fehler {} {
    ...
    if {$error == 1} {
      ...
      set sum [expr $wbegin($edit_exp) + $wup1($edit_exp) + $wtop($edit_exp) + \
               $wdown($edit_exp) + $wntop($edit_exp) + $wup2($edit_exp)]
      if {$sum <= $top($edit_exp)} {
  ...
  method uebernehmen {to from} {
    ...
    # falls to editiertes Experiment ist
    if {$to == $edit_exp} {
      set_graph_edit_exp
  ...
  method set_topstrom {num {val ""}} {
    set wtopstrom($num) $val
    database set_$this wtopstrom $num $val
  }


  method set_wbegin {num {val ""}} {
    set wbegin($num) $val
    database set_wedel wbegin $num $val
  }


  method set_wup1 {num {val ""}} {
    set wup1($num) $val
    database set_wedel wup1 $num $val
  }


  method set_wtop {num {val ""}} {
    set wtop($num) $val
    database set_wedel wtop $num $val
  }


  method set_wdown {num {val ""}} {
    set wdown($num) $val
    database set_wedel wdown $num $val
  }


  method set_wntop {num {val ""}} {
    set wntop($num) $val
    database set_wedel wntop $num $val
  }


  method set_wup2 {num {val ""}} {
    set wup2($num) $val
    database set_wedel wup2 $num $val
  }
  ...
  method set_topstromval {num {val ""}} {set wtopstrom($num) $val}
  method set_wbeginval {num {val ""}} {set wbegin($num) $val}
  method set_wup1val {num {val ""}} {set wup1($num) $val}
  method set_wtopval {num {val ""}} {set wtop($num) $val}
  method set_wdownval {num {val ""}} {set wdown($num) $val}
  method set_wntopval {num {val ""}} {set wntop($num) $val}
  method set_wup2val {num {val ""}} {set wup2($num) $val}
  method get_topstrom {num} {return $btopstrom($num)}
  method set_wend {num {val ""}} {set wend($num) $val}
  ...
  protected title Wedel
  protected initial w
  protected sel_proc wedelsel
  protected scale_proc wedelscale
  protected magnete {}
}
