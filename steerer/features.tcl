...
proc bltResetBindings { graph type } {
    ...
    set cmds {}
    foreach elem $all {
	if [string match "$type,$graph,*" $elem] {
	    lappend cmds $bindings($elem)
	}
    }
...
proc ResetCrosshairs { graph } {
    global Crosshair_xtag

    catch {destroy $graph.x}
    catch {$graph tag delete $Crosshair_xtag}
    set points "Inf -Inf"
    label $graph.x -text {0.0} -fg black -width 10 -relief ridge
    set Crosshair_xtag [$graph tag create window $points -anchor se -pathname $graph.x]
}

proc SetCrosshairs { graph } {
    global bindings
    
    ResetCrosshairs $graph
    $graph crosshairs set on
    set bindings(<Motion>,$graph,crosshairs) {
        set info [%W invtransform %x %y]
        set xval [format "%%.1f" [lindex $info 0]]
        %W crosshairs configure -position @%x,%y
        %W.x config -text $xval
    }
    bltResetBindings $graph <Motion>
}
